...
async function processSiteJob(job) {
  const site = job && job.data ? job.data : {};
  ...
  try {
    ...
    if (result && result.invalid) {
      ...
      logger.info("updates-worker: selector invalid", { siteId, reason: result.reason });
      return { changed: false, invalid: true };
    }

    if (!result || !result.changed) {
      logger.info("updates-worker: no changes", { siteId });
    ...
    for (const item of newItems) {
      ...
      await saveDetectedUpdate({
        ...
        title: item.title || "New update",
        link: item.link || "",
        latestContent: item.fingerprint || item.latestContent || ""
      });
      pendingBatch.push({
        siteName: site.name,
        title: item.title,
    ...
    logger.info("updates-worker: job success", {
      jobId: job.id,
      siteId,
      savedCount
    });
...
siteWorker.on("completed", (job) => {
  logger.info("updates-worker: completed", { jobId: job.id, siteId: job.data && job.data.siteId });
});
...
logger.info("updates-worker: started", {
