...
/**
 * Structured section editor — parse [Section: …] text ↔ typed section models.
 * Keep public/assets/js/sectionEditorModel.js in sync.
 */
...
/**
 * Important Dates section: date rows, paragraphs, and list items (order preserved).
 * @param {string[]} lines
 */
...
function textBlockHasContent(text) {
...
/**
 * @param {string} text
 * @returns {Array<{ id: string, name: string, forceTable: boolean, contentType: string, payload: object }>}
 */
...
/**
 * @param {Array<{ name: string, forceTable?: boolean, contentType: string, payload: object }>} sections
 * @returns {string}
 */
...
function createEmptySection(name = "New Section", contentType = CONTENT_TYPES.PARAGRAPH) {
  return {
    id: newSectionId(),
    name,
    forceTable: contentType === CONTENT_TYPES.TABLE,
    contentType,
    payload: defaultPayloadForType(contentType)
  };
}
