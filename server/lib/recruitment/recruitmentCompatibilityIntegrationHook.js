...
/**
 * Phase 93 — Recruitment Compatibility Integration Hook.
 *
 * Feature-flagged, advisory-only hook that invokes the Phase 91 workflow
 * integration coordinator from inside the recruitment compatibility layer via
 * the Phase 94 observation registry. Observations are stored in internal
 * WeakMaps only — public compatibility results and pipeline outcomes are
 * never mutated.
 *
 * Flow: Compatibility Layer → Compatibility Integration Hook → Phase 91 Coordinator
 *
...
const COMPATIBILITY_INTEGRATION_HOOK_PHASE = 93;

const COMPATIBILITY_INTEGRATION_ENTITY = "recruitment_compatibility_integration_hook";

const COMPATIBILITY_INTEGRATION_METADATA = Object.freeze({
  phase: COMPATIBILITY_INTEGRATION_HOOK_PHASE,
...
const COMPATIBILITY_INTEGRATION_DESCRIPTOR = Object.freeze({
  entity: COMPATIBILITY_INTEGRATION_ENTITY,
  domain: "recruitment",
  phase: COMPATIBILITY_INTEGRATION_HOOK_PHASE,
  description:
    "Advisory compatibility hook invoking the Phase 91 workflow integration coordinator.",
  featureFlagId: WORKFLOW_INTEGRATION_FLAG_ID,
  metadata: COMPATIBILITY_INTEGRATION_METADATA
});
 ...
 * @param {Object|null|undefined} [compatibilityContext]
 * @returns {Readonly<Object>}
 */
function buildCompatibilityCoordinatorContext(pipelineOutcome, input, compatibilityContext) {
  const source = isPlainObject(input) ? input : {};
  let compatibility = isPlainObject(compatibilityContext) ? compatibilityContext : null;

  if (compatibility == null) {
    const { peekRecruitmentCompatibility } = require("./recruitmentCompatibilityLayer");
    compatibility = peekRecruitmentCompatibility(pipelineOutcome);
  }
 ...
 * @returns {Readonly<Object>}
 */
function buildCompatibilityDiagnostics(existingDiagnostics, integrationResult, coordinatorContext) {
  ...
  const appendResult = appendExecutionStage(existingTrace, {
    ...
    message: "Compatibility workflow integration coordination (advisory)",
    detail: {
      hookPhase: COMPATIBILITY_INTEGRATION_HOOK_PHASE,
      coordinatorPhase: WORKFLOW_INTEGRATION_COORDINATOR_PHASE,
...
/**
 * Attach advisory compatibility diagnostics without mutating public fields.
 *
 * @param {Object|null|undefined} pipelineOutcome
 * @param {Object|null|undefined} diagnostics
 * @returns {Readonly<Object>|null}
 */
function attachRecruitmentCompatibilityDiagnostics(pipelineOutcome, diagnostics) {
...
/**
 * Read advisory compatibility diagnostics for a pipeline outcome, if any.
 *
 * @param {Object|null|undefined} pipelineOutcome
 * @returns {Readonly<Object>|null}
 */
function peekRecruitmentCompatibilityDiagnostics(pipelineOutcome) {
...
/**
 * Read advisory compatibility integration result for a pipeline outcome, if any.
 *
 * @param {Object|null|undefined} pipelineOutcome
 * @returns {Readonly<Object>|null}
 */
function peekRecruitmentCompatibilityIntegration(pipelineOutcome) {
  if (!isPlainObject(pipelineOutcome)) {
    return null;
  }
...
/**
 * Invoke Phase 91 coordinator from the compatibility layer when the master flag is enabled.
 * Never throws. Never mutates public pipeline outcome or compatibility fields.
 *
 * @param {Object|null|undefined} pipelineOutcome
 * @param {Object|null|undefined} input
 * @param {Object|null|undefined} [compatibilityContext]
 * @returns {Readonly<Object>|null}
 */
function attachRecruitmentCompatibilityIntegration(
  pipelineOutcome,
  input,
  ...
  try {
    const coordinatorContext = buildCompatibilityCoordinatorContext(
      pipelineOutcome,
      source,
      compatibilityContext
    );
    const integrationResult = getOrCreateWorkflowObservation(pipelineOutcome, () => {
      ...
      return coordinateRecruitmentWorkflowIntegration(coordinatorContext);
    });
    if (integrationResult == null) {
      return null;
    }
    const existingDiagnostics =
      peekRecruitmentCompatibilityDiagnostics(pipelineOutcome) ??
      peekWorkflowDiagnostics(pipelineOutcome);
    const compatibilityDiagnostics = buildCompatibilityDiagnostics(
      existingDiagnostics,
      integrationResult,
      coordinatorContext
    );

    diagnosticsByPipelineOutcome.set(pipelineOutcome, compatibilityDiagnostics);
...
module.exports = {
  ...
  buildCompatibilityCoordinatorContext,
