...
/**
 * Package 5F — Integration regression: Programs 4–5E surfaces unchanged.
 *
 * Confirms Package 5F did not wire runtime routes or alter Program 4/5A–5E
 * admin pages / APIs. Publish readiness authorization remains advisory-only.
 */
...
describe("Package 5F does not change Program 4–5E production surfaces", () => {
  test("publish-readiness route is not activated", async () => {
    const response = await request(app).get("/admin/publish-readiness");
    expect([404, 302, 401]).toContain(response.status);
    expect(
      fs.existsSync(path.join(root, "private/admin-publish-readiness.html"))
    ).toBe(false);

    const alt = await request(app).get("/admin/authorization-panel");
  ...
  test("prior Program 5 advisory routes remain unwired", async () => {
    ...
    const resolution = await request(app).get("/admin/candidate-resolution");
    expect([404, 302, 401]).toContain(resolution.status);
  });
  ...
  test("Program 4 editorial and shared preview surfaces still present", () => {
    ...
    expect(nav).not.toContain("/admin/authorization-panel");
  });

  test("advisory publish readiness module exists without express wiring", () => {
    const indexSrc = read(
      "server/lib/recruitment/publishReadinessAuthorization/index.js"
    );
    expect(indexSrc).toMatch(/Package 5F/);
    ...
    expect(framework.safetyBoundaries.deploymentDenied).toBe(true);
    expect(framework.safetyBoundaries.automaticApprovalDenied).toBe(true);
  ...
  test("Package 5A–5E product facades still load", () => {
    const {
    ...
    const p5e = getControlledCandidateResolutionFramework();
    expect(p5e.packageCode).toBe("5E");
    expect(p5e.advisoryOnly).toBe(true);
  });
});
