...
/**
 * Local-safe: classify sources, verify 1–2 GREEN candidates, bounded dry-run.
...
const OUT =
  process.env.ACC_DRYRUN_OUT ||
  path.join(__dirname, "../.tmp-dryrun-probe/acc_bounded_local_dryrun_report.json");
...
async function main() {
  ...
  // Prefer already-approved UPPSC official GREEN source for local readiness.
  candidates.sort((a, b) => {
    const score = (row) => {
      if (/uppsc/i.test(String(row.name || "")) || /uppsc\.up\.nic\.in/i.test(String(row.url || ""))) {
        return 0;
      }
      return 1;
    };
  ...
  const selected = candidates.slice(0, 1); // exactly one bounded dry-run
  ...
  try {
    ...
    setDryRunMode(false, { updatedBy: "acc_bounded_local_dryrun_restore" });
    writeControlPlane({
      mode: AUTOMATION_MODES.DORMANT,
      updatedBy: "acc_bounded_local_dryrun_restore"
    });
  }
...
main().catch((err) => {
  ...
  try {
    setDryRunMode(false, { updatedBy: "acc_bounded_local_dryrun_fatal_restore" });
    writeControlPlane({ mode: AUTOMATION_MODES.DORMANT, updatedBy: "acc_bounded_local_dryrun_fatal_restore" });
  } catch (_) {}
  process.exit(1);
}).then(() => {
  process.exit(0);
